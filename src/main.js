...
function loadStoreEntries(raw) {
  ...
  entries = parseStoreEntries(raw).map((entry) => {
    ...
    const normalizedEntry = {
      ...
      wrapEnabled: Boolean(entry.wrapEnabled),
...
function addClipboardText(text, source = 'Clipboard') {
  ...
  const entry = {
    ...
    wrapEnabled: Boolean(existing?.wrapEnabled),
...
ipcMain.handle('entries:update', async (_event, nextEntry) => {
  ...
  entries[index] = {
    ...
    wrapEnabled:
      typeof nextEntry.wrapEnabled === 'boolean' ? nextEntry.wrapEnabled : Boolean(current.wrapEnabled),
    pinned: nextPinned,
